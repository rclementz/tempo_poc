...
def create_patch_span(event):
    ...
    with tracer.start_span(f"patchset {event['patchSet']['number']}",start_time=time,links=[trace.Link(parent)]) as ps:
...
def complete_patch_span(event): 
    ...
    with tracer.start_span(f"patchset {patch_nr_to_complete}",start_time=st,links=[trace.Link(parent)]) as ps:
        ps.set_attribute("Project",f"{event['change']['project']}") 
        ps.set_attribute("Subject",f"{event['change']['subject']}") 
        ps.set_attribute("URL",f"{event['change']['url']}") 
